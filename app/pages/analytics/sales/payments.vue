<template>
	<ZPagePanel id="analytics-sales-payments" :title="$t('pages.analyticsSalesPayments')">
		<template #toolbar>
						...
						<div class="bg-linear-to-r from-primary/5 to-primary/10 border-l-4 border-primary px-6 py-4" :class="{ 'border-t border-neutral-200': index > 0 }">
							<div class="flex items-center justify-between">
								...
								<div class="flex items-center gap-2 text-sm font-semibold text-primary">
									<span>{{ $t('pages.totalLabel') }}: {{ formatCurrency(group.net_amt, group?.items[0]?.currency_code) }}</span>
								</div>
							</div>
						</div>
