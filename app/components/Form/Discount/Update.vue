			...
			<div class="lg:col-span-9">
				...
				>
						...
						<div class="space-y-6 py-2 px-4">
							...
							<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
								<UFormField :label="$t('form.code')" name="code">
									...
									<UInput v-model="formState.code" disabled :placeholder="$t('components.discountForm.codePlaceholder')" />
								</UFormField>
					...
					<UCard id="section-discount-rule" class="shadow-md scroll-mt-4">
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2 px-4">
							<UFormField :label="$t('components.discountForm.ruleType')" name="rule_type" required>
								<USelect
									:model-value="formState.rule_type"
									:items="ruleTypeItems"
							...
							<UFormField
								v-if="formState.rule_type !== DiscountRuleType.FREE_SHIPPING"
								:label="$t('components.discountForm.ruleValue')"
								name="rule_value"
								required
							>
								<UInput
									v-model.number="formState.rule_value"
									type="number"
									min="0"
									:max="formState.rule_type === DiscountRuleType.PERCENTAGE ? 100 : undefined"
									step="0.01"
								>
									<template #trailing>
								...
								<p v-if="formState.rule_type === DiscountRuleType.PERCENTAGE" class="text-xs text-muted mt-1">
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2 px-4">
							...
							<UFormField :label="$t('components.discountForm.allocation')" name="allocation">
								...
								<USelect v-model="formState.allocation" :items="allocationItems" value-attribute="value" class="w-full max-w-md" />
							</UFormField>
						</div>
					</UCard>
					...
					<UCard id="section-discount-conditions" class="shadow-md scroll-mt-4">
						...
						<div v-if="!formState.conditions?.length" class="py-6 px-4 text-sm text-muted">
							{{ $t('components.discountForm.noConditions') }}
						</div>

						<div v-else class="space-y-6 py-2 px-4">
							<div v-for="(cond, index) in formState.conditions" :key="index" class="rounded-lg border border-default p-4 space-y-4">
...
import { applyDiscountToFormState, emptyDiscountFormEditableState } from '~/utils/discount-form-state';
...
const discountActive = computed({
	get: () => !formState.is_disabled,
	set: (value: boolean) => {
		formState.is_disabled = !value;
	},
});
...
const addCondition = () => {
	const list = formState.conditions ?? [];
	formState.conditions = [...list, emptyCondition()];
};

const removeCondition = (index: number) => {
	const list = [...(formState.conditions ?? [])];
	list.splice(index, 1);
	formState.conditions = list;
};
