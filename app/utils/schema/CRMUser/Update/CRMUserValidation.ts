...
export function UpdateCRMUserValidation(t: TranslateFn) {
	return z.object({
		...
		phone_no: z.string({ message: t('validation.crmUser.phoneNumberRequired') }).regex(/^\d{6,15}$/, {
			message: t('validation.crmUser.phoneNumberInvalid'),
		}),
		...
		staff_department_id: z.number().nullable().optional(),
	});
}
