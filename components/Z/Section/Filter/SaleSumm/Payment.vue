	...
	<UCard>
		<div class="w-full flex flex-col gap-4">
			<div class="gap-2 sm:flex-jbetween-icenter w-full sm:w-[60%]">
				...
				<ZSelectMenuDateFilterType v-model:filter-type="sale_summ_payments.filter.filter_type" />

				<div class="flex-col-start gap-2">
					<ZSelectMenuDate :date="sale_summ_payments.filter.start_date" placeholder="Start Date" @update:date="sale_summ_payments.filter.start_date = $event" />
					<ZSelectMenuDate
						v-if="sale_summ_payments.filter.filter_type === 'between'"
						:date="sale_summ_payments.filter.end_date"
						placeholder="End Date"
						:min-date="sale_summ_payments.filter.start_date"
						@update:date="sale_summ_payments.filter.end_date = $event"
					/>
				</div>
			</div>
