  ...
  actions: {
    async getDashboardSummary(range?: Range) {
      ...
      try {
        ...
        const message =
          (err as ErrorResponse).message ?? "Failed to load sales summary";
    ...
    async updateSaleSummPage(page: number) {
      ...
      if (
        this.sale_summ.current_page < 0 ||
        this.sale_summ.total_data === this.sale_summ.data.length
      ) {
        return;
      }
      ...
      try {
        ...
        if (this.sale_summ.filter.date_range.end) {
          const dateFilter = `(biz_date between '${getFormattedDate(this.sale_summ.filter.date_range.start!, "yyyy-MM-dd")}' and '${
            this.sale_summ.filter.date_range.end
              ? getFormattedDate(
                  this.sale_summ.filter.date_range.end!,
                  "yyyy-MM-dd",
                )
        ...
        const { data, "@odata.count": total } =
          await $api.summSales.getSummSales({
            $filter: filter,
    ...
    async updateSaleItemSummPage(page: number) {
      ...
      if (
        this.sale_summ_items.current_page < 0 ||
        this.sale_summ_items.total_data === this.sale_summ_items.data.length
      ) {
        return;
      }
      ...
      try {
        ...
        if (this.sale_summ_items.filter.date_range.end) {
          const dateFilter = `(biz_date between '${getFormattedDate(this.sale_summ_items.filter.date_range.start!, "yyyy-MM-dd")}' and '${
            this.sale_summ_items.filter.date_range.end
              ? getFormattedDate(
                  this.sale_summ_items.filter.date_range.end!,
                  "yyyy-MM-dd",
                )
        ...
        const { data, "@odata.count": total } =
          await $api.summSales.getSummSalesItems({
            $filter: filter,
            $orderby: "biz_date desc",
            $count: true,
            ...
            $skip:
              (this.sale_summ_items.current_page - 1) *
              this.sale_summ_items.page_size,
          });
        if (data) {
    ...
    async updateSalePaymentSummPageSize(size: number) {
      ...
      if (
        this.sale_summ_payments.page_size > this.sale_summ_payments.total_data
      ) {
    ...
    async updateSalePaymentSummPage(page: number) {
      ...
      if (
        this.sale_summ_payments.current_page < 0 ||
        this.sale_summ_payments.total_data ===
          this.sale_summ_payments.data.length
      ) {
        return;
      }
    ...
    async getSalePaymentSummary() {
      ...
      try {
        ...
        if (this.sale_summ_payments.filter.payment_status) {
          ...
          filter = filter
            ? `${filter} and ${paymentStatusFilter}`
            : paymentStatusFilter;
        }
        ...
        if (this.sale_summ_payments.filter.date_range.end) {
          const dateFilter = `(biz_date between '${getFormattedDate(this.sale_summ_payments.filter.date_range.start!, "yyyy-MM-dd")}' and '${
            this.sale_summ_payments.filter.date_range.end
              ? getFormattedDate(
                  this.sale_summ_payments.filter.date_range.end!,
                  "yyyy-MM-dd",
                )
        ...
        const { data, "@odata.count": total } =
          await $api.summSales.getSummSalesPayments({
            $filter: filter,
            $orderby: "biz_date desc",
            $count: true,
            ...
            $skip:
              (this.sale_summ_payments.current_page - 1) *
              this.sale_summ_payments.page_size,
          });
      ...
      try {
        ...
        if (this.sale_summ_customer.filter.date_range.end) {
          const dateFilter = `(biz_date between '${getFormattedDate(this.sale_summ_customer.filter.date_range.start!, "yyyy-MM-dd")}' and '${
            this.sale_summ_customer.filter.date_range.end
              ? getFormattedDate(
                  this.sale_summ_customer.filter.date_range.end!,
                  "yyyy-MM-dd",
                )
        ...
        const { data } = await $api.summSales.getSummSalesCustomers({
          ...
          $skip:
            (this.sale_summ_customer.current_page - 1) *
            this.sale_summ_customer.page_size,
        });
