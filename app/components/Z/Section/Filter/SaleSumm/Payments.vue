<template>
	<div class="w-full py-4">
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-3 items-center">
			...
			<div class="flex flex-col gap-1.5">
				<label class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ $t('components.filter.paymentStatus') }}</label>
				<ZSelectMenuPaymentStatus v-model:payment-status="filter.payment_status" @update:model-value="handlePaymentStatusChange" />
			</div>
		...
		<div v-if="hasActiveFilters" class="flex flex-wrap gap-2 items-center">
			...
			<UBadge v-if="filter.payment_status" color="info" variant="subtle" size="sm" @click="clearFilter('payment_status')">
				{{ $t('components.filter.paymentStatus') }}: {{ capitalizeFirstLetter(filter.payment_status) }}
				<UIcon name="i-heroicons-x-mark" class="w-3 h-3 ml-1 cursor-pointer" />
			</UBadge>
