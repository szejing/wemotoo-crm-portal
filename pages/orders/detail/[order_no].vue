<template>
	<div>
		...
		<div class="flex-between w-full mt-4">
			...
			<div>
				<UBadge v-if="order?.order_status == OrderStatus.COMPLETED" color="green" size="lg">COMPLETED</UBadge>
				...
				<table class="w-full">
					...
					<tbody>
						<tr v-for="item in items" :key="item.prod_code" class="border-b">
							<td class="cell">{{ item.prod_code }}</td>
							<td class="cell">{{ item.prod_name }}</td>
							<td class="cell">
